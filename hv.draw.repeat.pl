  ...
  foreach my $key (sort keys %hash_cover){
    foreach my $sec_key (sort keys %{$hash_cover{$key}}){
      ...
      if($q_or_s eq "q"){
        ...
        if($sec_key*$myblock>$querychr2plen{$key}){my $posx2=$left_curb*1.25+$frame_width;}
        else{my $posx2=$posx1+$myblock/$scale_ratio1;}
      ...
      #sbjct
        ...
        if($sec_key*$myblock>$sbjctchr2plen{$key}){my $posy2=$top_curb*1.25+$frame_height;}
        else{my $posy2=$posy1+$myblock/$scale_ratio2;}
